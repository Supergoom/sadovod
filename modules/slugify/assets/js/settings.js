document.addEventListener( 'DOMContentLoaded', () => {
	class Tables {
		 ...
		 */
		constructor() {
			this.OPTIONS_FORM_SELECTOR = '#slugify-options';
			...
			this.CURRENT_STUB_ID = 'slugify-current';
			this.CURRENT_NAV_TAB_CLASS = 'nav-tab-current';
			this.ACTIVE_NAV_TAB_CLASS = 'nav-tab-active';
			this.ACTIVE_TABLE_CLASS = 'active';
			this.EDIT_LABEL_ID = 'slugify-edit-label';
			this.EDIT_LABEL_ERROR_CLASS = 'slugify-edit-label-error';
			this.plusButton =
		 ...
		 */
		addMessageLines() {
			this.successMessage = this.addMessageLine( 'slugify-success' );
			this.errorMessage = this.addMessageLine( 'slugify-error' );
		}
		 ...
		 */
		moveTabs() {
			const tabs = document.querySelector( '.slugify-settings-tabs' );
			if ( tabs ) {
		...
		hideTables() {
			...
			this.getTables().map( ( table, index ) => {
				table.classList.add( 'slugify-table' );
