...
function true_show_profile_fields($user)
{
    ...
    // добавляем поле ID пользователя в GIS
    $gis_user_id = get_the_author_meta('gis_user_id', $user->ID);
    if (!empty($gis_user_id)) {
        echo '<tr><th><label for="gis_user_id">ID пользователя в GIS</label></th>
 	<td><input type="text" name="gis_user_id" id="gis_user_id" value="' . esc_attr($gis_user_id) . '" class="regular-text" /></td>
	</tr>';
    }

    // добавляем поле ID группы в GIS
    $gis_group_id = get_the_author_meta('gis_group_id', $user->ID);
    if (!empty($gis_group_id[0])) {
        echo '<tr><th><label for="gis_group_id">ID группы в GIS</label></th>
 	<td><input type="text" name="gis_group_id" id="gis_group_id" value="' . esc_attr($gis_group_id[0]) . '" class="regular-text" /></td>
	</tr>';
    }

    // добавляем поле Ник в GIS
    $gis_username = get_the_author_meta('gis_username', $user->ID);
    if (!empty($gis_username)) {
        echo '<tr><th><label for="gis_username">Ник в GIS</label></th>
 	<td><input type="text" name="gis_username" id="gis_username" value="' . esc_attr($gis_username) . '" class="regular-text" /></td>
	</tr>';
    }
