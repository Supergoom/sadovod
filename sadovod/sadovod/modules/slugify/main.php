...
class Main
{
    ...
    public function __construct()
    {
        ...
        include_once('menu/converter.php');
    ...
    {
        wp_enqueue_style('slugify-main', get_stylesheet_directory_uri() . '/modules/slugify/assets/css/slugify.css');
    }
    ...
    public function sanitize_title($title, $raw_title = '', $context = '')
    {
        ...
        if ($this->is_term) {
            ...
            $sql = $wpdbx->prepare(
                "SELECT slug FROM $wpdbx->terms t LEFT JOIN $wpdbx->term_taxonomy tt
    ...
    public function sanitize_filename($filename, $filename_raw)
    {
        ...
        if (seems_utf8($filename)) {
            $filename = (string) mb_strtolower($filename);
        }
    ...
    protected function get_wpml_locale()
    {
        ...
        return (isset($this->wpml_languages[$language_code]) ?
    ...
    public function prepare_in($items, $format = '%s')
    {
        ...
        if ($how_many > 0) {
            ...
            $prepared_in = $wpdbx->prepare($prepared_format, $items);
        }
