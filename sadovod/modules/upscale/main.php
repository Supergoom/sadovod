...
class Main
{
    ...
    public function image_resize_dimensions($value, $orig_w, $orig_h, $dest_w, $dest_h, $crop)
    {
        ...
        if ($crop) {
            ...
            if (!$new_w) {
                $new_w = parse_int($new_h * $aspect_ratio);
            }

            if (!$new_h) {
                $new_h = parse_int($new_w / $aspect_ratio);
            }
    ...
    public function constrain_dimensions($current_width, $current_height, $max_width = 0, $max_height = 0)
    {
        ...
        if (parse_int($current_width * $larger_ratio) > $max_width || parse_int($current_height * $larger_ratio) > $max_height)
        ...
        $w = parse_int($current_width * $ratio);
        $h = parse_int($current_height * $ratio);
